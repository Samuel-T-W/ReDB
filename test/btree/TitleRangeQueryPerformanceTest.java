 ...
 *
 * <p>
 * Method 1 – Table Scan: iterate every page/slot, filter records by title
 * range. Method 2 – Index Scan: use the B+ tree title index to get RIDs, then
 * fetch the matching rows from the data pages.
 *
 * <p>
 * Results are written to title_range_query_results.csv. Run plot_results.py to
 * produce the two figures.
 */
public class TitleRangeQueryPerformanceTest {
	...
	private static final String MOVIES_IDX = "movies_title_perf.idx";
	private static final String RESULTS_CSV = "report/title_range_query_results.csv";

	private static final int MOVIE_ID_LEN = 9;
	private static final int TITLE_LEN = 30; // also used as B+ tree KEY_SIZE
	...
	/**
	 * Scans every slot on every data page and counts records whose title falls in
	 * [startTitle, endTitle] (byte-level comparison, same as the B+ tree).
	 *
	...
	private long tableScan(byte[] startBytes, byte[] endBytes, int numDataPages, int[] recordsPerPage, BufferManager bm)
		...
		for (int pid = 0; pid < numDataPages; pid++) {
			...
			for (int slot = 0; slot < numRec; slot++) {
				...
				if (Arrays.compare(titleBytes, startBytes) >= 0 && Arrays.compare(titleBytes, endBytes) <= 0) {
					count++;
				}
			}
	...
	/**
	 * Uses the B+ tree title index to obtain RIDs for the range, then fetches each
	 * matching record from the data pages.
	 *
	 * @return number of matching records
	 */
	private long indexScan(String startTitle, String endTitle, BTreeManager btree, BufferManager bm)
			throws IOException {
		Iterator<RecordId> rids = btree.rangeSearch(fixedAsciiKey(startTitle, TITLE_LEN), fixedAsciiKey(endTitle, TITLE_LEN));
		long count = 0;
		while (rids.hasNext()) {
	...
	public void testRangeQueryPerformance() throws Exception {
		...
		bm.register(new IndexEntry(MOVIES_IDX, TITLE_LEN));
		...
		int totalMovies = allTitles.size();
		System.out.printf("Loaded %d movies into %d data page(s).%n", totalMovies, numDataPages);

		// --- 3. Build B+ tree title index ---
		BTreeManager btree = new BTreeManager(BTREE_DEGREE, MOVIES_IDX, bm, TITLE_LEN);
		for (int i = 0; i < totalMovies; i++) {
			btree.insert(fixedAsciiKey(allTitles.get(i), TITLE_LEN), allRids.get(i));
		}
		...
		// Sort titles in the same byte order the B+ tree uses
		List<String> sortedTitles = new ArrayList<>(allTitles);
		Collections.sort(sortedTitles);
		int n = sortedTitles.size();

		// Fixed start: the lexicographically smallest title
		String startTitle = sortedTitles.get(0);
		byte[] startBytes = toFixedBytes(startTitle, TITLE_LEN);

		double[] selectivities = {0.001, 0.005, 0.01, 0.05, 0.1, 0.2, 0.5};

		// Pre-compute end titles for each selectivity
		String[] endTitles = new String[selectivities.length];
		...
		for (int s = 0; s < selectivities.length; s++) {
			...
			endTitles[s] = sortedTitles.get(endIdx);
			endBytesArr[s] = toFixedBytes(endTitles[s], TITLE_LEN);
		}
		...
		for (int s = 0; s < selectivities.length; s++) {
			...
			for (int r = 0; r < MEASURE_REPS; r++) {
				...
				t0 = System.nanoTime();
				count2 = indexScan(startTitle, endTitle, btree, bm);
			...
			assertEquals(count1, count2,
					String.format("count mismatch at selectivity %.3f: table=%d index=%d", sel, count1, count2));
		...
		System.out.println("Run: python3 title_plot_results.py");
